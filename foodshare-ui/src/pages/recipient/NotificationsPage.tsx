...
const ICON_MAP: Record<string, { icon: typeof Bell; color: string; bg: string }> = {
  ...
  PAYMENT:      { icon: CreditCard,      color: '#7c3aed', bg: '#ede9fe' },
...
const TABS = [
  ...
  { key: 'REPORT', label: 'Hệ thống & Cảnh báo' },
];
...
export default function RecipientNotificationsPage() {
  ...
  const filteredNotifications = notifications.filter(n => {
    ...
    if (activeTab === 'REPORT') return n.notificationType === 'REPORT' || n.notificationType === 'SYSTEM' || n.notificationType === 'REVIEW';
